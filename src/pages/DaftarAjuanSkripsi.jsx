...
import TambahDataSkripsi from "./TambahAjuanSkripsi";

Modal.setAppElement("#root");

export default function DaftarAjuanSkripsi() {
  ...
  const handleSearch = () => {
    ...
    const filtered = tasks.filter((task) => {
      const tujuanPenelitian = task.tujuanPenelitian || "";
      const researcherName = task.researcherName || "";
      const nomorSurat = task.nomorSurat || "";

      return (
        tujuanPenelitian.toLowerCase().includes(lowercasedKeyword) || // Search in "ajuan"
        researcherName.toLowerCase().includes(lowercasedKeyword) || // Search in "nama"
        nomorSurat.toLowerCase().includes(lowercasedKeyword) // Search in "nomor surat"
      );
    });
  ...
  const handleEditClick = (task, index) => {
    setEditingTask({
      index,
      id: task.id,
      name: task.tujuanPenelitian,
      date: task.timestamp,
      nama: task.researcherName,
  ...
  const handleEditTask = async (updatedTask) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_REACT_APP_SERVER}/api/penelitian/update/${
          updatedTask.id
        }`,
        updatedTask
      );
  ...
  const handleViewClick = (task) => {
    navigate("/detail-ajuan-penelitian/tesis/skripsi", {
      state: { formData: task },
    });
  };
      ...
      <div className="md:pl-64 flex flex-col flex-1 min-h-screen bg-[#EEF5FF] pt-5 px-2">
        <div className="max-w-full mx-auto px-2 sm:px-4 md:px-6 lg:px-8">
          <h1 className="text-2xl font-bold text-gray-900 mb-8 mt-0 md:mt-5">
            Daftar Ajuan Rekomendasi Penelitian/Tesis/Skripsi
          </h1>
        ...
        {isDateFilterModalOpen && (
          ...
          >
            <div className="relative p-6 rounded-lg max-w-lg mx-auto bg-white">
              <button
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  className="w-full border border-gray-300 rounded-md p-2 text-gray-700 bg-gray-50 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition duration-150"
                />
              </div>
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className="px-6 py-2 bg-blue-600 text-white font-semibold rounded-md shadow hover:bg-blue-700 transition duration-150"
                >
                  Terapkan
                </button>
              </div>
              ...
              <div className="relative bg-white rounded-lg shadow">
                ...
                <div className="flex items-center justify-between p-4 md:p-5 border-t border-gray-200 rounded-b">
                  ...
                  <button
                    ...
                    className="text-white bg-[#2A629A] hover:bg-[#003285] font-medium rounded-full border border-gray-300 text-sm md:text-base px-4 py-1.5 text-center"
                  >
                    Terapkan
                  </button>
                </div>
